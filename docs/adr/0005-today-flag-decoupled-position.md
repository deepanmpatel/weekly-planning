...
- Lazy cleanup at `GET /tasks/today` flips `is_today=false` for `done` tasks whose `completed_at` predates midnight America/Los_Angeles of the date 2 business days back (weekdays only — no holiday calendar). Done tasks linger on Today for two working days before being evicted on the next fetch. Mirrored in [demoStore.ts](../../web/src/lib/demo/demoStore.ts). The cutoff helper (`staleDoneCutoffUtcIso()`) is the first TZ-anchored cutoff in the codebase — reuse the same `Intl.DateTimeFormat`-based pattern if more land.
